...
import { Shield, CheckCircle, MapPin, Building, Award, TrendingUp, Zap, Globe } from 'lucide-react';
...
const InsurancePage = () => {
  ...
  const nationalPolicies = policies.filter(policy => policy.policy_type === 'National');
  const statePolicies = policies.filter(policy => policy.policy_type === 'State-Level');
  ...
  const statesPolicies = [
    ...
    {
      state: "Maharashtra", 
      coverage: "Significant financial incentives for green hydrogen production",
      features: [
  ...
  const subsidyComponents = [
    {
      ...
      description: "Protection for capital subsidies, interest subvention, and SGST reimbursement",
      features: ["Automated milestone verification", "Blockchain-based fund release"]
    },
    {
      title: "SIGHT Programme Insurance", 
      description: "Protection for incentives under Strategic Interventions for Green Hydrogen Transition",
      features: ["Electrolyzer manufacturing incentives", "Fuel cell production subsidies", "Green hydrogen production incentives"]
    },
    {
      title: "Export Incentive Insurance",
      description: "Protection for export-related subsidies and incentives", 
      features: ["Currency fluctuation protection", "International compliance guarantee", "Shipping and logistics coverage"]
    }
  ];

  const premiumStructure: PremiumStructure[] = [
    { type: "Manufacturing Projects", range: "2.5% - 4.0%", basis: "of project value" },
    { type: "Production Projects", range: "1.8% - 3.2%", basis: "of annual production value" },
    { type: "Infrastructure Projects", range: "3.0% - 5.5%", basis: "of infrastructure investment" },
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Insurance Solutions</h1>
                <p className="text-sm text-green-600">Comprehensive Green Hydrogen Subsidy Protection</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 bg-green-100 px-4 py-2 rounded-full">
              <Globe className="w-4 h-4 text-green-600" />
              <span className="text-sm font-medium text-green-800">India Market</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative bg-gradient-to-r from-green-600 to-emerald-600 text-white py-16">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-bold mb-4">Secure Your Green Hydrogen Investments</h2>
          <p className="text-xl text-green-100 max-w-3xl mx-auto">
            Comprehensive insurance coverage for green hydrogen subsidies and incentives across India, 
            powered by blockchain technology for complete transparency and automated processing.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'national' && (
          <div className="space-y-6">
            {nationalPolicies.map((policy) => (
              <div key={policy.$id} className="bg-white rounded-2xl shadow-xl p-8 border border-green-100">
                <div className="flex items-center mb-6">
                  <Award className="w-8 h-8 text-green-600 mr-3" />
                  <h3 className="text-3xl font-bold text-gray-900">{policy.policy_name}</h3>
                </div>
                
                <div className="grid md:grid-cols-2 gap-8 mb-8">
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50 p-6 rounded-xl">
                    <h4 className="text-lg font-semibold text-green-800 mb-2">Total Programme Outlay</h4>
                    <p className="text-3xl font-bold text-green-600">{policy.total_outlay_covered}</p>
                    <p className="text-sm text-gray-600 mt-2">Under SIGHT Programme</p>
                  </div>
                  
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 p-6 rounded-xl">
                    <h4 className="text-lg font-semibold text-blue-800 mb-2">Policy Description</h4>
                    <p className="text-lg text-blue-600">{policy.description}</p>
                  </div>
                </div>

                {policy.eligibility_summary && (
                  <div className="bg-gray-50 p-6 rounded-xl">
                    <h4 className="text-xl font-semibold text-gray-900 mb-4">Eligibility Criteria</h4>
                    <p className="text-gray-700">{policy.eligibility_summary}</p>
                  </div>
                )}
        ...
        {activeTab === 'states' && (
          <div className="space-y-6">
            <h3 className="text-3xl font-bold text-gray-900 text-center mb-8">State-Level Green Hydrogen Policies</h3>
            <div className="grid md:grid-cols-2 gap-6">
              {statePolicies.map((policy) => (
                <div key={policy.$id} className="bg-white rounded-2xl shadow-xl p-6 border border-green-100 hover:shadow-2xl transition-shadow">
                  <div className="flex items-center mb-4">
                    <div className="bg-green-100 p-3 rounded-lg mr-4">
                      <MapPin className="w-6 h-6 text-green-600" />
                    </div>
                    <h4 className="text-xl font-bold text-gray-900">{policy.target_region || 'All Regions'}</h4>
                  </div>
                  
                  <div className="bg-green-50 p-4 rounded-lg mb-4">
                    <p className="text-green-800 font-medium">{policy.total_outlay_covered}</p>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {policy.eligibility_summary && (
                      <div>
                        <h5 className="font-semibold text-gray-900 mb-2">Eligibility Criteria:</h5>
                        <p className="text-gray-700 text-sm">{policy.eligibility_summary}</p>
                      </div>
                    )}
        ...
        {activeTab === 'companies' && (
          <div className="space-y-6">
            <h3 className="text-3xl font-bold text-gray-900 text-center mb-8">Leading Insurance Providers</h3>
            <div className="space-y-6">
        ...
        {activeTab === 'components' && (
          <div className="space-y-6">
            <h3 className="text-3xl font-bold text-gray-900 text-center mb-8">Subsidy Insurance Components</h3>
            <div className="grid lg:grid-cols-3 gap-6">
              {subsidyComponents.map((component, index) => (
                <div key={index} className="bg-white rounded-2xl shadow-xl p-6 border border-green-100 hover:shadow-2xl transition-shadow">
            ...
            <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl p-8 text-white mt-12">
              <h4 className="text-2xl font-bold mb-6">Technology Integration Features</h4>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h5 className="text-lg font-semibold mb-3">Digital India Integration</h5>
                ...
                <div>
                  <h5 className="text-lg font-semibold mb-3">Blockchain-Enabled Features</h5>
                  <ul className="space-y-2 text-green-100">
        ...
        {activeTab === 'pricing' && (
          <div className="space-y-6">
            <h3 className="text-3xl font-bold text-gray-900 text-center mb-8">Premium Structure</h3>
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-green-100">
              <div className="bg-green-600 text-white p-6">
                <h4 className="text-xl font-bold">Indian Market Premium Rates</h4>
                <p className="text-green-100">Competitive pricing for comprehensive coverage</p>
              </div>
                  ...
                  {premiumStructure.map((item, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-green-50 transition-colors">
                      ...
                      <div className="text-right">
                        <p className="text-lg font-bold text-green-600">{item.range}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-gradient-to-r from-green-600 to-emerald-600 rounded-2xl p-8 text-white text-center">
              <h4 className="text-2xl font-bold mb-4">Ready to Secure Your Green Hydrogen Project?</h4>
              <p className="text-green-100 mb-6">Get started with TrustChain's blockchain-powered insurance solutions</p>
      ...
      <div className="bg-gray-900 text-white py-12 mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <p className="text-3xl font-bold text-green-400">{nationalPolicies.length}</p>
              <p className="text-gray-300">National Policies</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-green-400">{statePolicies.length}</p>
              <p className="text-gray-300">State Policies</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-green-400">{policies.length}</p>
